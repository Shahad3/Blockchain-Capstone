...
contract Ownable {
    ...
    modifier onlyOwner(){
        require(msg.sender == owner, "This functionality is only available  to the owner.");
        _;
    }
...
contract Pausable is Ownable{
    ...
    //---------------- Events ----------------//
    event Paused(address tAddress);
    event Unpaused(address tAddress);

    //--------------- Modifiers --------------//
    modifier whenNotPaused(){
        require(_paused == false, "It's currently paused");
        _;
    }

    modifier paused(){
        require(_paused == true, "It's currently not paused");
        _;
    }

    function setter(bool _value) public onlyOwner{
        _paused = _value;
    }
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender), "require the msg sender to be the owner");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to != address(0), "Unvalid address");
